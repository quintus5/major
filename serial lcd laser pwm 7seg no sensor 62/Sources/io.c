...
// predefined strings
const unsigned char maxverti[]   = "\nmax vertical angle (1-5)\r\n";
const unsigned char maxhori[]    = "\nmax horizontal angle(1-5)\r\n";
const unsigned char validangle[] = "\nthis is alrite i guess, move on...\r\n";
const unsigned char errorangle[] = "\nwhat are u doing? get it right!\r\n";
...
interrupt 21 void SCI1_ISR(void){     // row 21 of interrupt vector table, (SCI1)
...
// max and min is mirrored across 90 degree
  while(1){
    printstring(maxverti);

    max_angle_t = waitinput();
    if(max_angle_t >= '1' && max_angle_t <= '5'){
      max_angle_t = ((max_angle_t-'0')*5);
      min_angle_t = 90 - max_angle_t;
      max_angle_t += 90;
      printstring(validangle);
      break;
    }
    else{
      printstring(errorangle);
    }
  }

  while(1){
    printstring(maxhori);
    max_angle_h = waitinput();
